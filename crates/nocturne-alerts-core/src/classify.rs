...
//! | leaf | low | high |
//! |---|---|---|
//! | `threshold.direction` | `below` | `above` |
//! | `rate_of_change.direction` | `falling` | `rising` |
//! | `predicted.operator` | `<` `<=` | `>` `>=` |
//! | `glucose_bucket.buckets` | `very_low`, `low` | `high`, `very_high` |
//!
//! `predicted.operator` `==` is neither. For `glucose_bucket`, `in_range` and
//! `tight_range` are neither, and a set leaf can span both sides.
//!
